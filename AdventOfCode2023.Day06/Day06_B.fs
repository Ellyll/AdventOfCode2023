...
let parse (lines: string array) =
    let raceTime =
        (int64 <| regWhiteSpace.Replace(regRaces.Replace(lines[0], "$1"), ""))
        * 1L<ms>
    let raceDistance =
        (int64 <| regWhiteSpace.Replace(regRaces.Replace(lines[1], "$1"), ""))
        * 1L<mm>
...
let getNumberOfWaysToWin (time: int64<ms>,distance: int64<mm>) =
    seq { 1L..(int64(time)-1L) }
    |> Seq.map (fun t -> t*1L<ms>)
    |> Seq.choose (fun t ->
        let speed = (int64 t)*1L<mm/ms>
        let travelTime = ((int64 time) - (int64 t))*1L<ms>
...
let run () =
    printfn $"%s{getNow()} Running Day06_B..."
    ...
    stopwatch.Stop()
    printfn $"%s{getNow()} Day 06 Part 2:- Result: %i{result} (Time taken: %d{stopwatch.ElapsedMilliseconds}ms)"
